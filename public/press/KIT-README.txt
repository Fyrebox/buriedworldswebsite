BURIED WORLDS VR — PRESS KIT
============================

A VR treasure-hunting game for Meta Quest.
Out now in Early Access on the Meta Horizon Store.

  Store    https://www.meta.com/experiences/buried-worlds/1129006663640647/
  Site     https://www.buriedworlds.com/
  Kit      https://www.buriedworlds.com/press
  Contact  press@buriedworlds.com

FACTS
  Store title    Buried Worlds (the store lists it without "VR")
  Price          US$14.99 (Meta shows local currency by region)
  Released       26 August 2026, in Early Access
  Devices        Meta Quest 2, Quest 3, Quest 3S, Quest Pro
  Developer      Cyril Gaillard, working alone
  Studio         Bellare Studios (Melvia Pty Ltd), Victoria, Australia
  Built with     Unity 6, URP, OpenXR, Meta XR SDK
  Genres         Adventure, Simulation — single player, no combat
  Languages      English, French
  Comfort        Moderate; seated or standing; teleport or free movement
  Rating         IARC 13+
  Size           462 MB
  Internet       Not required once installed

CONTENTS
  screenshots/   Eight 1920x1080 JPEGs. Captions are on the web version
                 of this kit at buriedworlds.com/press
  art/           Key art, cover art, wordmark (transparent), app icon

GETTING THE HISTORY RIGHT
  Hoxne, 1992    Eric Lawes was searching for a hammer lost by his tenant
                 farmer, Peter Whatling, when he found the hoard. The hammer
                 was recovered too and is held with the hoard in the British
                 Museum, Room 49. The game recreates the hammer.
  Coloma, 1848   James Marshall's find at Sutter's Mill, January 1848,
                 started the California Gold Rush.
  Carcassonne    UNESCO-listed since 1997.
  Bolonia        The Roman town of Baelo Claudia, Andalusia.
  The detector   Gold and lead deliberately overlap on the target ID scale,
                 because that is what the ground does to you in reality.

USE
  These assets may be used freely in reviews, previews, news coverage,
  videos and streams of Buried Worlds VR, including monetised ones. No
  permission needed. Please do not alter the logo or present the artwork
  as your own work.

  The trailer is at buriedworlds.com/press — a 1080p master is available
  on request.
